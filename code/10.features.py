...
def word_order(verb_index, sent):
	...
	for i in range(len(core_list)):
		...
		#	print(verb_index, core_list, tok)
		#	print(sent)
		#	print('\n')
...
def full_analyze(file_handle):
	...
	with io.open(file_handle, encoding = 'utf-8') as f:
		...
		while sent is not None:
			...
			if len(check) > 1:
				if int(check[1]) - int(check[0]) > len(sent) - int(check[1]):
					sent = sent[int(check[1]) : ]
				else:
					sent = sent[ : int(check[1])]
			...
			all_dependencies.append([tok[7] for tok in sent])
			
			for tok in sent:
				if tok[7] not in unique_dependencies:
					unique_dependencies.append(tok[7])
				...
				if tok[3] in function_POS:
					...
					function_lemma_list.append(tok[2])
...
if __name__ == '__main__':
	...
	for corpus in corpus_list:
		print(corpus)
		outfile = io.open(args.output + corpus + '_features.txt', 'w', encoding = 'utf-8')
		...
		directory_choice = directory_list[0]
		...
		file_name = str(first_file) + '.conllu'
		all_info, word_order_dictionary, subordinate_order_dictionary, verb_features, aux_features = full_analyze(args.input + corpus + '/' + directory + '/' + file_name)
		...
		for folder in os.listdir(args.input + corpus + '/'):
			print(folder)
			if os.path.isdir(args.input + corpus + '/' + folder + '/') is True and folder not in ['Korean', 'Hindi', 'Japanese', 'German']:
				for file in os.listdir(args.input + corpus + '/' + folder + '/'):
					lang = folder

					if file.endswith('.conllu'): 
						try:
							all_info, word_order_dictionary, subordinate_order_dictionary, verb_features, aux_features = full_analyze(args.input + corpus + '/' + folder + '/' + file)
