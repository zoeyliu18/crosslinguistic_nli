...
from ..utils.field import Field, BertField, RemBertField, XLMRField
...
class Parser():
    ...
    def train(self, train, dev, test,
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            ### Saving the model from each epoch ###
            self.save(args.path + '_' + str(epoch))
    ...
    def load(cls, name_or_path='', lang='en', cache_dir=os.path.expanduser('~/.cache/diaparser'), **kwargs):
        ...
        #    if args.bert.startswith('google/rembert'):
        #        tokenizer = RemBertField.tokenizer(args.bert)
        #    if args.bert.startswith('xlm'):
        #        tokenizer = XLMRField.tokenizer(args.bert)
