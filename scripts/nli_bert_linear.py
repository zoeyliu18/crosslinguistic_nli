...
class DAProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, binary_pred):
        """See base class."""
        return self._create_examples(
            os.path.join(data_dir, "TOEFL_bert_train.txt"), "train", binary_pred)

    def get_dev_examples(self, data_dir, binary_pred, inference=False):
        """See base class."""
        if not inference:
            return self._create_examples(
               os.path.join(data_dir, "TOEFL_bert_test.txt"), "dev", binary_pred, inference)
        else:
            return self._create_examples(
...
#                       }
        ...
        LANGUAGE_MAP = {'ja':'ja', 'it': 'it', 'te': 'te', 'zh': 'zh', 'ar': 'ar', 'hi': 'hi', 'de': 'de', 'ko': 'ko', 'fr': 'fr', 'es': 'es', 'tr': 'tr'
                       }
    ...
    def _create_examples(self, filename, set_type, binary_pred, inference=False):
        ...
        with open(filename) as fp:
            ...
            #    if das.split(";")[0].strip() in ['statement', 'back-channeling', 'opinion', 'pos_answer', 'appreciation', 'yes_no_question', 'neg_answer', 'command', 'thanking', 'open_question_factual', 'open_question_opinion', 'comment', 'dev_command']:
                guid = "%s-%s" % (set_type, i)
                toks = line.strip().split(' ## ')
                text_a = toks[0].split('\t')
                text_a = ' '.join(sent for sent in text_a)
...
def accuracy(out, labels):
    ...
    return np.sum(outputs == labels) ### compare whether index is the same ###
...
def main():
    ...
    parser.add_argument("--exist", default=None, type=str, required=False,
                        help="path to existing pretrained dialogue act prediction models.")
    ...
    parser.add_argument("--finetune",
                        action='store_true',
                        help="Whether to finetune using existing model.")
...
#    num_labels = num_labels_task[task_name]
    ...
    if args.finetune:
        model = BertForSequenceClassification.from_pretrained(args.exist, num_labels=num_labels)
    ...
    if (args.do_eval or args.do_inference) and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        if args.do_inference:
            ...
            output_inference_file = os.path.join(args.output_dir, "dev_results.txt")
            inference_writter = open(output_inference_file, "w")
        ...
        for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            if args.binary_pred:
                ...
                logits_backup = logits
                logits = logits.detach().cpu().numpy()
                label_ids = label_ids.to('cpu').numpy()
                tmp_eval_accuracy = accuracy(logits, label_ids)

                if args.do_inference:
                #    top_k_value, top_k_ind = torch.topk(logits_backup, 1)
                    prediction_indexes = np.argmax(logits, axis = 1)
                    for idx in prediction_indexes:
                        inference_writter.write(label_list[idx] + "\n")
                else:
